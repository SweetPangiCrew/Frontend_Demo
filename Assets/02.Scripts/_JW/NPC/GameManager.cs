...
public class GameManager : MonoBehaviour
{
    ...
    private int stepTime = 18; // Í≤åÏûÑ ?úÍ∞Ñ?ºÎ°ú 18Î∂?ÎßàÎã§ ?§ÌÖù???ÖÎç∞?¥Ìä∏ ??
    ...
    void Start()
    {
        ...
        if (gameName != "")
        {
            ...
            step = Database.Instance.StartStep; // Ï¥àÍ∏∞?îÎäî 0?ºÎ°ú, game load?úÍ±∞?ºÎ©¥ ?§Î¶Ñ.
        }
        else
        {
            gameName = "game1";
    ...
    private IEnumerator InvokePerceive()
    {
        ...
        int pStep = 1; //?¥Î? base?êÏÑú perceiveÍ∞Ä 0???àÏùå.
        while (true)
        {
            ...
            //server manage?êÏÑú ?úÎ≤ÑÍ∞Ä ???¥Î†∏?ÑÎïå
            ...
            {
                //step???¨ÎùºÍ∞Ä???Ä?¥Î∞ç???îÏùÑ ?????úÎ≤àÎß??∏Ï∂ú
                ...
                //serverÍ∞Ä Perceive ?åÏùº??Î∞õÏïò????
            ...
            yield return new WaitForSeconds(1f); //10Ï¥àÎ? 18Î°??òÎà¥????0.55556?¥Îùº 0.5??Î∞òÎ≥µ?òÎ©¥ 1Î∂??®ÏúÑ Í≤åÏûÑ ?úÍ∞Ñ??Î™®Îëê Ï≤¥ÌÅ¨??
        }
    }
